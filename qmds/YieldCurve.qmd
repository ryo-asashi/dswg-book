...
YieldCurveはパラメトリックなイールドカーブを生成することができるパッケージです。Nelson-Siegel、またはSvenssonのモデルによりイールドカーブを生成します。また、このパッケージには、連邦準備銀行（FRB）と欧州中央銀行（ECB）の金利のデータセットも含まれています。
...
```
ECBのデータセットも同様に取得可能です。こちらは満期が3ヶ月と6ヶ月、および1年から30年の日時データで、範囲は2006/12/28から2009/7/23です。
...
Nelson.Siegel関数では、Nelson-Siegelモデルによるパラメータを推定できます。具体的には、金利データをインプットすると、以下の算式で表されるNelson-Siegelモデルのイールドカーブについて、パラメータ$\beta_{0t}$（水準）、$\beta_{1t}$（傾き）、$\beta_{2t}$（曲率）、$\lambda$（こぶの位置）を推定します。次のコードでは先ほど取得したFRBのデータから最初の3ヶ月分と最後の3ヶ月分のパラメータを推定しています。

$$
y_t(\tau) = \beta_{0t} 
          + \beta_{1t}\frac{1-\exp(-\lambda_\tau)}{\lambda_\tau} 
          + \beta_{2t}\left( \frac{1-\exp(-\lambda_\tau)}{\lambda_\tau} 
          - \exp(-\lambda_\tau) \right)
$$
...
$$
y_t(\tau) = \beta_{0} 
          + \beta_{1}\exp\left(-\frac{\tau}{\lambda_1}\right) 
          + \beta_{2}\frac{\tau}{\lambda_1} \exp\left(-\frac{\tau}{\lambda_1}\right) 
          + \beta_{3}\frac{\tau}{\lambda_2} \exp\left(-\frac{\tau}{\lambda_2}\right) 
$$
...
\begin{equation}
  \begin{split} 
  y_t(\tau) &= \beta_{0} 
            + \beta_{1}\frac{1-\exp\left(-\frac{\tau}{\lambda_1}\right)}{\frac{\tau}{\lambda_1}} \\
            &+ \beta_{2}\left[ \frac{1-\exp\left(-\frac{\tau}{\lambda_1}\right)}{\frac{\tau}{\lambda_1}}
                              -\exp\left( -\frac{\tau}{\lambda_1} \right) \right] \\
            &+ \beta_{3}\left[ \frac{1-\exp\left(-\frac{\tau}{\lambda_2}\right)}{\frac{\tau}{\lambda_2}}
                                -\exp\left( -\frac{\tau}{\lambda_2} \right) \right]
  \end{split}
\end{equation}
$$

金利データをインプットすると、Svenssonモデルのイールドカーブについて、パラメータ$\beta_{0}$、$\beta_{1}$、$\beta_{2}$、$\beta_{3}$、$\lambda_1$、$\lambda_2$を推定します。次のコードでは、Nelson-Siegel関数と同様に、FRBのデータから最初の3ヶ月分と最後の3ヶ月分のパラメータを推定しています。

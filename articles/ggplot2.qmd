...
ggplot2は、一貫性のある文法を用いてグラフを作成することのできるパッケージであり、Rでモダンな分析環境を構築するためのパッケージ群である「tidyverse」に含まれます。 なお、ggplot2については、アクチュアリージャーナル第１１２号内の「Rを用いたデータの可視化技術解説書」の付録２でも紹介されています。

```{r ggplot2}
library(ggplot2)
library(dplyr) # パイプ演算子（%>%）を使うために必要
```

### グラフの作成方法

ggplot2では、+演算子を利用してggplot関数に様々な関数を接続することでグラフを作成します。 以下は、geom_point関数を用いて、Rのデータセットiris（アヤメの花びらやがく片の長さなどに関するデータセット）を用いた散布図を作成しています。
...
ggplot2では、グラフの重ね合わせも+演算子で実施できます。先ほど作成した散布図に対して、再度geom_point関数を接続すれば重ね合わせたグラフが作成可能です。

以下は、アヤメの種類別のがく片の長さと幅を計算したiris_meanを用いて、散布図を重ね合わせています（表示されたグラフの赤い点が平均値です）

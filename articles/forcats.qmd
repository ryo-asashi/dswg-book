...
```{r forcats}
library(forcats)
library(dplyr) # パイプ演算子（%>%）を使うために必要。statsパッケージの関数（filterなど）が上書きされる
library(ggplot2) # 利用例の説明に活用
```
...
なお、ベクトル内の要素が重複している場合、Levelsではまとめて表示されます。以下の例では、「c」が重複していますが、Levelsで表示されるcは１つだけです。
...
factor型のlevelsは、例えば、データの可視化に活用できます。
...
まず、可視化するデータセットをarrange関数を用いてSepal.Widthが大きい順に並べ替えた結果を表示してみます。結果のtibbleの並べ替えはできていますが、棒グラフには反映されていません。
...
次に、iris_mean_by_speciesのSpeciesのlevelsをsetosa、virginica、verscolorの順番に変更した後に棒グラフを作成すると、結果がSepal.Widthが大きい順番に表示されていることが確認できます。

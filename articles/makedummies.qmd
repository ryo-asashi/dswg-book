...
冒頭6行のレコード、およびサマリーを確認すると次のとおりです。
...
引数 `basal_level` に `TRUE` を与えることで、基準となるカテゴリに対応する列も作られるようになります[^encoding]。

[^encoding]: 基準となるカテゴリに対応する列が無いもの（`basal_level` が `FALSE` の場合）をダミーエンコーディング、基準となるカテゴリに対応する列があるもの（`basal_level` が `TRUE`の場合）をワンホットエンコーディングと呼び分けることがあります。
...
引数 `numerical` に列名を与えることで、その列は単純に1, 2, 3, …とカテゴリ別の整数に変換するように変更できます[^encoding2]。

[^encoding2]: このような変換をラベルエンコーディングといいます。
...
`makedummies`関数は、このような順序付きfactor型に対しても通常のfactor型と同様に動作します。
...
```{r modelmatrix}
#順序付きfactor型に対してはデフォルトでは多項式対比を使用
head(model.matrix(~ . , data = df_all))
#povertyに対して多項式対比を使用しない設定をした場合
head(model.matrix(~ . , data = df_all, contrasts.arg = list(poverty = contr.treatment)))
```
...
head(dummy_cols(df_all,
                #処理対象列の指定
                select_columns = c("poverty","religion","degree","country"),
                #元の列を残さない
